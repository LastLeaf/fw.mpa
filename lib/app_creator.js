...
module.exports = function(appRouter, path){
	...
	app.start = function(cb){
		...
		startProcessId = pid;
		clientCache.create(app, pid, routers['client'].list(), function(){
			if(startProcessId !== pid) {
				if(startProcessId) clientCache.remove(app, pid);
				else clientCache.apply(app, pid);
				return;
			}
			initDb(function(){
				if(startProcessId !== pid) {
