...
module.exports = function(appRouter, appPath){
	...
	app.socket = function(sock){
		if(!sock.app.enabled || sock.app.destroyed) {
			sock.end();
			return;
		}
	...
	var moduleLoader = function(dirsInfo, cb){
		app.loadedModules = {};
		var nextDir = function(){
			if(!dirsInfo.length) return cb();
			var dirInfo = dirsInfo.shift();
			fs.readdir(dirInfo.dir, function(err, files){
				if(err) return nextDir();
				var nextFile = function(){
					if(!files.length) return nextDir();
					var file = files.shift();
					fs.stat(dirInfo.dir + '/' + file, function(err, stat){
						if(err || !stat) return nextFile();
						if(stat.isDirectory()) {
							// load as a dir
							fs.exists(dirInfo.dir + '/' + file + '/index.js', function(exists){
								if(!exists) return;
								if(fw.debug) console.log('Loading module: ' + dirInfo.prefix + '/' + file);
								var func = loadScript(app, 'module', dirInfo.prefix, file, dirInfo.dir + '/' + file + '/index.js');
								func(function(obj){
									app.loadedModules[dirInfo.prefix + '/' + file] = obj;
									nextFile();
								});
							});
							return;
						}
						if(file.slice(-3) === '.js') return nextFile();
						// load as a file
						if(fw.debug) console.log('Loading module: ' + dirInfo.prefix + '/' + file);
						var func = loadScript(app, 'module', dirInfo.prefix, file, dirInfo.dir + '/' + file);
						func(function(obj){
							app.loadedModules[dirInfo.prefix + '/' + file] = obj;
							nextFile();
						});
			...
			clientRouter.update(routers.page.list(), function(){
				...
				httpRouterStart(function(){
					...
					initDb(function(){
						...
						// TODO load other server components after modules
						moduleLoader(routers.module.list(), function(){
							if(startProcessId !== pid) {
								(closeDb || closeDbDefault)(cb);
								return;
							}
	...
	// get appconfig
	args.unshift(app);
	require(path.relative(__dirname, process.cwd() + '/' + appPath)).apply(fw, args);
	return app;
};
