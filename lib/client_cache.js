	...
	return function(dirs, cb){
		...
		if(fw.mode !== 'cache') {
			...
			var next = function(){
				...
				var dirInfo = dirs.shift();
				var prefix = dirInfo.prefix;
				var src = dirInfo.dir;
				utils.walkFileTree(src, function(file, cb){
					if(file.slice(-5) === '.subm') {
						var submPath = prefix + '/' + file.slice(0, -5);
						app.clientSubmodules.push({
							path: submPath,
							src: src + '/' + file,
						});
					}
