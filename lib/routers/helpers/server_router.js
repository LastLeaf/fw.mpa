...
module.exports = function(app, dirType, prefix, dir, routeFunc, reloadApp, cb){
	var routes = {};
	...
	listener.on('.js', function(type, relPath){
		if(reloadApp && type === 'init') return app.restart();
		...
		if(type === 'remove') {
			...
			routes[rpcPath] = loadScript(app, 'rpc', rpcPath, codePath);
