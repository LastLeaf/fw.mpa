...
module.exports = function(app, dirType, prefix, dir, routeFunc, cb){
	var routes = {};
	var tmplManager = app.serverRoute.tmpl.addDir(dirType, prefix, dir);
	var tmplPending = 0;
	...
	listener.on('.js', function(type, relPath){
		...
		if(type === 'remove') {
			...
			routes[rpcPath] = loadScript(app, dirType, prefix, rpcPath, codePath);
			if(type !== 'init') console.info(routes[rpcPath]);
			if(typeof(routes[rpcPath]) === 'undefined') delete routes[rpcPath];
			if(fw.debug && type !== 'init') console.log('Code Updated (' + dirType + '): ' + rpcPath);
		}
	});
		...
		if(type === 'remove') {
			...
			fs.readdir(codePath.replace(/\.tmpl$/, '.locale'), function(err, files){
				...
				tmplManager.set(relPath, codePath, files, function(){
					...
					if(fw.debug && type !== 'init') console.log('Tmpl Updated (' + dirType + '): ' + relPath);
				});
			});
		}
	});
		...
		fs.exists(tmplPath, function(exists){
			...
			if(type === 'remove') {
				...
				tmplManager.set(relPath, tmplPath, [locale], function(){
					if(fw.debug && type !== 'init') console.log('Locale Updated (' + dirType + '): ' + relPath);
				});
			}
		});
	});
