...
module.exports = function(dir, keepWatch, cb){
	...
	var watcherArgs = null;
	if(keepWatch) {
		watchr.watch({
			path: dir,
			persistent: false,
			next: function(err, watcher){
				if(!watcher) watcher = { close: function(){} };
				if(startChange) cb(err, watcher);
				else watcherArgs = [err, watcher];
			},
	...
	utils.walkFileTree(dir, function(relPath){
		...
		startChange = true;
		if(watcherArgs) cb(watcherArgs[0], watcherArgs[1]);
	});
	return eventEmitter;
};
