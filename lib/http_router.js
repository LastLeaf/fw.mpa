...
module.exports = function(app, cb){
	...
	expressApp.use('/~fw', staticCache);
	expressApp.use('/~client', staticCache);
	...
	if(fw.debug) {
		expressApp.use('/~client', function(req, res, next){
			// TODO check req path and file ext
			if(req.method !== 'GET' && req.method !== 'HEAD') {
				res(404);
				return;
			}
			var reqPath = req.path;
			var file = app.route.client(reqPath);
			if(!file) {
				res(404);
				return;
			}
			...
			preprocessor(app, file, function(files){
				if(file.slice(-5) === '.tmpl') {
					...
					res(files['.'+req.language+'.js'] || files['.js']);
				} else {
					// get the only file
					for(var k in files) {
						res(files[k]);
	...
	expressApp.get('*', function(req, res){
		if(req.path.slice(0, 2) === '/~') {
			if(fw.debug && req.path === '/~fw/routes.js') {
				...
				// TODO generate client routes file
				res.send(app.clientRoutesFile);
