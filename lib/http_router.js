...
var clientFiles = function(app, cb){
	if(app.debug) {
		...
		var clientFileHandler = function(file, type, match){
			...
			// find .min
			if(file.slice(-7) === '.min.js') return;
			if(file.slice(-8) === '.min.css') return;
			var minFile = '';
...
exports.route = function(app, express, cb){
	...
	if(app.debug) {
		...
		if(app.mode === 'CACHE') {
			...
			if(fs.existsSync(app.config.client.favicon))
				fs.writeFileSync(app.config.path.cache+'/fw/'+app.config.client.favicon, fs.readFileSync(app.config.path.rc+'/'+app.config.client.favicon));
			// compress fw code
	...
	app.get('*', function(req, res){
		...
		} else if(app.debug && req.path === '/~fw/'+app.config.client.favicon) {
			// loading logo
			res.sendfile(app.config.path.rc + '/' + app.config.client.favicon);
