...
var updateRoutesFile = function(app){
	...
	for(var base in client) {
		for(var id in client[base]) {
			...
			if(!app.debug) {
				var files = [];
				files = files.concat(route.main).concat(route.lib).concat(route.style).concat(route.tmpl);
				for(var i=0; i<files.length; i++) {
					if(!files[i].minify) continue;
					var fd = fs.openSync(files[i].minify, 'w');
					if(files[i].src.constructor === Array)
						for(var j=0; j<files[i].src.length; j++)
							fs.writeSync(fd, fs.readFileSync(files[i].src[j]) + '\n');
					else
						fs.writeSync(fd, fs.readFileSync(files[i].src[j]) + '\n');
					fs.closeSync(fd);
				}
...
exports.route = function(app, express, cb){
	...
	if(app.debug) {
		...
		if(app.mode === 'CACHE') {
			// write cache/fw
			rmdirp(app.config.path.cache + '/fw');
			if(!fs.existsSync(app.config.path.cache))
				fs.mkdir(app.config.path.cache);
			if(!fs.existsSync(app.config.path.cache + '/fw'))
				fs.mkdirSync(app.config.path.cache + '/fw');
			if(app.config.client.loadingLogo)
				fs.writeFileSync(app.config.path.cache+'/fw/'+app.config.client.loadingLogo, fs.readFileSync(app.config.path.rc+'/'+app.config.client.loadingLogo));
			if(fs.existsSync(app.config.path.rc+'/favicon.ico'))
				fs.writeFileSync(app.config.path.cache+'/fw/favicon.ico', fs.readFileSync(app.config.path.rc+'/favicon.ico'));
			// compress fw code
			...
			fs.writeFileSync(app.config.path.cache+'/fw/init.js', parseCopyright(str) + uglifyjs.minify(str, {fromString: true}).code);
			fs.writeFileSync(app.config.path.cache+'/fw/libs.js', fs.readFileSync(__dirname+'/client/libs.js'));
			var str = fs.readFileSync(__dirname+'/client/fw.js').toString('utf8');
			fs.writeFileSync(app.config.path.cache+'/fw/fw.js', parseCopyright(str) + uglifyjs.minify(str, {fromString: true}).code);
		}
		app.use('/~fw', express.static(app.config.path.cache + '/fw'));
		app.use('/~', express.static(app.config.path.cache + '/client'));
	}

	// index page generator for web
	var index = handlebars.compile(fs.readFileSync(__dirname+'/default/index.html').toString('utf8'));
	var indexPage = function(auth, lang, styles, title, content){
		return index({
			debug: app.debug || 0,
	...
	if(app.mode === 'CACHE') {
		var packaged = handlebars.compile(fs.readFileSync( __dirname+'/default/packaged_webapp.html' ).toString('utf8'));
		var html = packaged({
			debug: app.debug || 0,
			version: app.config.app.version,
			timeout: app.config.server.timeout,
		...
		fs.writeFileSync(app.config.path.cache+'/webapp.html', html);
	}

	// default routes
	app.get('*', function(req, res){
		if(app.debug && req.path === '/~fw/routes.js') {
			// routes file
			res.type('text/javascript');
			res.send(routesFile);
		} else if(app.debug && req.path === '/~fw/'+app.config.client.loadingLogo) {
			// loading logo
			res.sendfile(app.config.path.rc + '/' + app.config.client.loadingLogo);
			...
			app.render.path(app, req, req.path, 0, function(css, r){
				res.send(indexPage(app.sessionCookieVal(req.sessionID), req.language, css, r.title, r.content));
			});
		}
	});
