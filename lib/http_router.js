...
exports.route = function(app, express, cb){
	...
	if(app.debug) {
		...
		if(app.config.app.loadingLogo)
			fs.writeFileSync(app.config.path.cache+'/fw/'+app.config.app.loadingLogo, fs.readFileSync(app.config.path.rc+'/'+app.config.app.loadingLogo));
		...
		fs.writeFileSync(app.config.path.cache+'/fw/init.js', uglifyjs.minify(__dirname+'/client/init.js').code);
		fs.writeFileSync(app.config.path.cache+'/fw/libs.js', fs.readFileSync(__dirname+'/client/libs.js'));
		fs.writeFileSync(app.config.path.cache+'/fw/fw.js', uglifyjs.minify(__dirname+'/client/fw.js').code);
	...
	for(var i=0; i<app.config.app.locale.length; i++) {
		var loc = app.config.app.locale[i];
		var html = packaged({
			debug: app.debug || 0,
			version: app.config.app.version,
			timeout: app.config.server.timeout,
		...
		fs.writeFileSync(app.config.path.cache+'/webapp_'+loc+'.html', html);
	}
	...
	app.get('*', function(req, res){
		if(app.debug && req.path === '/~fw/routes.js') {
			...
			res.send(routesFile);
		} else if(app.debug && req.path === '/~fw/'+app.config.app.loadingLogo) {
			// loading logo
			res.sendfile(app.config.path.rc + '/' + app.config.app.loadingLogo);
			...
			if(req.query.lang)
				req.language = req.conn.language = req.query.a;
			res.type('text/javascript');
			...
			app.render.path(app, req, req.path, 0, function(css, r){
				res.send(indexPage(app.sessionCookieVal(req.sessionID), req.cookies['fw.lang'], css, r.title, r.content));
			});
		}
	});
