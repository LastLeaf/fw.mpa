...
module.exports = function(app){
	...
	var updateRoutes = function(client, page, cb){
		// calculate parent
		...
		fs.writeFile(app.config.app.cache + '/~fw/routes.js', app.clientRoutesFileContent, cb);
	};
	...
	var setRoute = function(base, id, oriObj, isAdd){
		...
		if(!isAdd && oriObj.redirect) {
			...
			if(r.charAt(0) !== '/') oriObj.redirect = ('/' + base + r).replace(/\/\.\//g, '/');
