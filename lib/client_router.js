...
module.exports = function(app){
	...
	var updateRoutes = function(client, page, cb){
		...
		for(var id in client) {
			...
			if(parent.indexOf('.') >= 0 || parent.indexOf('/') >= 0 || parent.indexOf('*') >= 0) {
				if(parent.charAt(0) !== '/') id = ('/' + route.b + id).replace(/\/\.\//g, '/').replace(/\/\.$/, '');
	...
	var setRoute = function(base, id, oriObj, isAdd){
		...
		if(id.indexOf('.') >= 0 || id.indexOf('/') >= 0 || id.indexOf('*') >= 0) {
			...
			if(id.charAt(0) !== '/') id = ('/' + base + id).replace(/\/\.\//g, '/').replace(/\/\.$/g, '');
		} else {
			id += ':/' + base;
		}
		...
		if(!isAdd && oriObj.redirect) {
			...
			if(r.charAt(0) !== '/') r = ('/' + base + r).replace(/\/\.\//g, '/').replace(/\/\.$/g, '');
		...
		var combineList = function(oldList, newList, defExt){
			...
			for(var i=0; i<res.length; i++) {
				...
				if(typeof(item) === 'object') {
					...
					} else if(item.src.slice(-defExt.length) === defExt) {
