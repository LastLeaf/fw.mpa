...
if(fw.auth) {
	...
	var serverAuth = localStorage['fw'] || '';
}
...
// select language
if(fw.mode === 'web') {
	fw.selectLanguage = function(str, cb){
		location.replace('/~conf/lang?a=' + encodeURIComponent(serverAuth) + '&s=' + str + '&r=' + encodeURIComponent(fw.getPath()));
	};
} else {
	fw.selectLanguage = function(str, cb){
		if(str)
			location.replace('webapp_'+str+'.html');
		else
...
var sock = (function(){
	...
	var init = function(){
		...
		selectRpcServer();
		tag.src = rpcServer + '~conf/sock.js' + ( fw.debug ? '?t=' + new Date().getTime() : '?v=' + fw.version ) + '&a=' + encodeURIComponent(serverAuth);
...
var loadSubpage = function(id, renderDepth, loadId, cb){
	...
	var stopStyles = requireStyles(route.base, route.style, function(){
		...
		// rendering
		if(renderDepth)
			fw._loadJs(fw.mainServerPrefix + '~render'+curPath+'?d='+renderDepth)._loadJs(function(){
...
fw.go = function(path){
	...
	if(typeof(path) === 'number') {
		...
		return loadPage(path, function(path){
			if(app.mode === 'web' && history.pushState) {
...
fw.redirect = function(path){
	fw.stopLoading();
	return loadPage(path, function(path){
		if(app.mode === 'web' && history.replaceState)
...
var routesLoaded = function(){
	...
	loadPage(path, function(newPath){
		if(path !== newPath && history.replaceState)
