...
var parseLoaderFiles = function(files){
	...
	for(var i=0; i<files.length; i++) {
		...
		if(typeof(file) === 'object') {
			if(userAgentDenied(file.userAgent)) continue;
			if(fw.debug) pending = pending.concat(file.src);
			else pending = pending.concat(file.minify || file.src);
		} else {
			pending.push(file);
		}
	}
...
var loadPage = function(path, cb){
	...
	for(var i=0; i<stack.length; i++) {
		...
		// compare args
		var keys = pgStack[i].route.k;
		console.info(keys);
		console.info(args);
		console.info(curArgs);
		if(!keys) continue;
...
addListener(document, 'click', function(e){
	...
	if(e.buttons > 1 || e.ctrlKey || e.metaKey || e.altKey || e.shiftKey) return;
