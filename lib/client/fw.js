...
var userAgentDenied = function(ua){
	if(!ua) return false;
	return !navigator.userAgent.match(new RegExp(ua));
};
...
var parseLoaderFiles = function(files){
	...
	for(var i=0; i<files.length; i++) {
		...
		if(typeof(file) === 'object') {
			if(userAgentDenied(file.ua)) continue;
...
var requireScriptsMain = function(base, files, cb){
	var pending = parseLoaderFiles(files);
...
var unloadSubpage = function(loadId){
	...
	removeStyles();
	console.log('Unloaded Sub-Page: ' + pg.routeId);
	...
	var stopStyles = requireStyles(route.base, route.style, function(){
		...
		var startRender = function(){
			...
			fw._loadJs(function(){
				...
				requireScriptsMain(route.base, route.main, function(){
					if(pageLoading !== loadId) return;
					pg.obj.emit('load');
