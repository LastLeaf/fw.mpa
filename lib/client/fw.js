...
var sock = (function(){
	...
	var init = function(){
		...
		tag.src = '/~conf/sock.js' + ( fw.debug ? '?t=' + new Date().getTime() : '?v=' + fw.version );
...
var requireScript = function(file){
	...
	fw
		...
		._loadJs(file + ( fw.debug ? '?t=' + new Date().getTime() : '?v=' + fw.version ))
		._loadJs(function(){
			if(scriptMain) {
...
var requireStyle = function(file, cb){
	...
	tag.rel = "stylesheet";
	tag.type = 'text/css';
	file += (fw.debug ? '?t=' + new Date().getTime() : '?v=' + fw.version);
	tag.href = file;
	document.head.appendChild(tag);
...
fw
	._loadJs('/~rc/routes.js' + ( fw.debug ? '?t=' + new Date().getTime() : '?v=' + fw.version ))
