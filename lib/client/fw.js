// Copyright 2013 LastFlower Team
'use strict';
...
fw.wrapper = document.getElementById('fw_wrapper');
...
var LocalStorageManager = function(id, version, defaultObj){
	try {
		if(localStorage[id]) {
			var obj = JSON.parse(localStorage[id]);
			if(obj._version !== version) {
				obj = shallowExtend(shallowExtend({}, defaultObj), obj);
				obj._version = version;
			}
		} else {
			var obj = deepExtend({}, defaultObj);
			obj._version = version;
		}
		var save = function(){
		...
		// fall back to common obj if not supported
		var obj = deepExtend({}, defaultObj);
		obj._version = version;
		var save = function(){};
		return {
...
fw.localData = LocalStorageManager(fw.config.localStorageId, fw.config.version, fw.config.localStorageDefault);

// template manager
window.tmpl = {};
fw.newTmpls = function(tmpls){
	for(var k in tmpls)
		window.tmpl[k] = Handlebars.template(tmpls[k]);
};
