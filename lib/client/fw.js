...
var requireScriptsMain = function(base, files, pgObj, subm, cb){
	...
	if(!loadingCount) cb();
	var loadedOne = function(exports){
...
var submManager = function(pg, info, cb){
	var destroyed = false;
	var childSubmCtrls = [];
	var childSubmResultsList = [];
	var childSubmResults = {};
	var addChild = function(url, info, cb){
		childSubmCtrls.push(submManager(pg, info, function(submodule){
			if(destroyed) return;
			requireScriptsMain(info.base, info.main, pg.obj, submodule, function(exports){
				childSubmResultsList.push(exports);
				childSubmResults[url] = exports;
				cb(exports);
			});
		}));
	};
	...
	var next = function(){
		...
		var detail = submDetail(info.base, children[i]) || submDetail(info.base, children[i] + '/index');
		if(!detail) throw(new Error('No submodule "' + children[i] + '" found.'));
		addChild(children[i], detail, function(){
			if(destroyed) return;
			i++;
			next();
		});
						...
						require: function(){
							...
							for(var i=0; i<len; i++) {
								...
								if(ext === 'css') {
									...
									var detail = submDetail(info.base, url) || submDetail(info.base, url + '/index');
									addChild(url, detail, pendingEnd);
								}
							}
						},
						import: function(path){
							var splitDot = path.lastIndexOf('.');
							var shortPath = path.slice(0, splitDot);
							var ext = path.slice(splitDot + 1);
							if(ext === 'js') {
								var file = requireScriptName(info.base, shortPath);
								return scriptsLoaded[file];
							} else if(ext === 'subm') {
								if(childSubmResults.hasOwnProperty(shortPath)) return childSubmResults[shortPath];
								return;
							}
							throw('Import method only supports scripts and submodules.');
						}
					});
				});
			});
...
fw._routes = function(propMap, tree, parser, cacheVers){
	...
	submDetail = function(base, id){
		if(id.charAt(0) !== '/') id = '/' + base + id;
	...
	var startLoading = function(){
		var startRender = function(){
			...
			pg.submCtrl = submManager(pg, route, function(res){
				...
				pg.obj.import = res.import;
				pg.obj.importAncestor = function(routeId){
					for(var cur = pg.obj.parent; cur; cur = cur.parent) {
						var curRouteId = cur.routeId;
						if(curRouteId === routeId) return cur.exports;
						if(curRouteId.charAt(0) !== '/') {
							curRouteId = curRouteId.slice(0, curRouteId.indexOf(':'));
							if(curRouteId === routeId) return cur.exports;
						}
					}
					return;
				...
				requireScriptsMain(route.base, route.main, pg.obj, null, function(exports){
					if(pageLoading !== loadId) return;
					pg.obj.exports = exports;
					pg.obj.emit('load');
