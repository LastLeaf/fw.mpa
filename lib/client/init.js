...
// ready function
bindReady(function(){
	fw._docHead = document.head || document.getElementsByTagName('head')[0];
	// select cache server
	if(fw.cacheServer) {
	...
	fw._loadJs = loadJs;
	fw
		._loadJs(fw.cacheFwPrefix + '/libs.js' + ( fw.debug ? '?t=' + new Date().getTime() : '?v=' + fw.fwVersion ))
		._loadJs(fw.cacheFwPrefix + '/fw.js' + ( fw.debug ? '?t=' + new Date().getTime() : '?v=' + fw.fwVersion ))
		._loadJs();
});
