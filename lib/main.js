...
var fwObjInit = function(){
	...
	fw.tmpl = function(file){
		if(!fw.currentLoading) return console.trace(new Error('Cannot load tmpl files out of require procedure.'));
		if(fw.currentLoading.type === 'module') return console.trace(new Error('Cannot load tmpl files from server modules.'));
	...
	fw.module = function(file){
		if(!fw.currentLoading) return console.trace(new Error('Cannot load modules out of require procedure.'));
		var app = fw.currentLoading.app;
		if(file.charAt(0) !== '/')
			file = fw.currentLoading.prefix + '/' + file;
		var mod = app.loadedModules[file] || app.loadedModules[file + '.js'];
		if(!mod) console.trace(new Error('Cannot find module "' + file + '".'));
		return mod;
	};
	// app loader
...
var main = function(config){
	...
	console.log('fw.mpa started at port(s) ' + ports.join(', ') + ', running in ' + ('"' + fw.mode + '"' || 'default') + ' mode.');
