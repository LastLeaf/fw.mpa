...
module.exports = function(config){
	...
	app.use(function(req, res, next){
		...
		req.db = app.db;
		// get raw body
		req.getRawBody = function(limit, callback){
			if(typeof(limit) === 'function') {
				callback = limit;
				limit = app.config.server.bodySizeLimit;
			} else if(limit > app.config.server.bodySizeLimit) {
				limit = app.config.server.bodySizeLimit;
			}
			getBody(req, {
				limit: limit,
	...
	// config
	app.config = deepExtend(require('./config.js'), config);
