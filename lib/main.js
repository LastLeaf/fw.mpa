...
var main = function(config){
	...
	for(var i=1; i < arguments.length; i++) {
		initialApps = initialApps.concat(arguments[i]);
	}

	// init fw object
	fw.restart = function(){
		if(fw.mode !== 'debug' && fw.mode !== 'cache' && fw.mode !== 'run') throw('Cannot restart in current mode (' + fw.mode + ').');
	...
	// config
	// TODO
	fw.config = utils.deepExtend(config);
	...
	var ports = app.config.server.port;
	...
	var initDb = function(cb){
		...
		dbEngine(app.config.db, function(db, sessionStore){
			app.db = db;
	...
	var route = function(){
		require('./http_router.js').route(app, express, function(){
			require('./sock_router.js').route(app, function(){
				for(var i=0; i<ports.length; i++) {
					servers[i].listen(ports[i], app.config.server.ip);
				}
				console.log('Server started at port(s) ' + ports.join(', ') + '.');
			});
		});
	};
